#include "/home/trakaros/sobel.h"
#include "math.h"
#define ABSDIFF(x,y)	((x>y)? x - y : y - x)
#define ABS(x)          ((x>0)? x : -x)

unsigned char window_buffer[3][3];
unsigned char line_buffer[3][MAX_WIDTH];
...
void convolution_sobel(AXI_STREAM& INPUT_STREAM, AXI_STREAM& OUTPUT_STREAM, int rows, int cols) {
	...
	//this is to avoid the error:ERROR: [XFORM 203-733] An internal stream...
	#pragma HLS dataflow
  ...
  //Sobel Filter
	...
	const char Gy[3][3] = { {-1,-2,-1},
	    ...
	    //edw na ta arxikopoihseis se miden!!!!!
